...
*********************************************************************
NOTE: If you have multiple versions of python on your machine make sure you install these dependencies so they can be used by the version of python which you intend to run our program with. If you are installing python for the first time you don’t need to worry about this. 
********************************************************************* 
...
If you have never run our program before we highly recommend you first run the program with demo.xml. This XML file is a sample preconfigured file. To run the simulation with this file follow the instructions under “Running the Program” and use demo.xml as the input file. Note that this part assumes that you have installed the software mentioned above in the Installation Instructions.
...
XML is a textual file format that is used to format data. This is the file format that our simulation uses to accept user input and simulation configurations. An XML file is composed of tags, these are the basic building blocks of the file. Each tag has a name and contents like so
...
There can be multiple levels to this nesting as in the case of our files. This nesting of tags is intended to reflect structure. Tags within another tag represent "parts" of that tag. For example in the case of our simulation the tower tag has subtags for height and width as shown below.
...
To fill out an XML file with actual data you need to go to each tag that has no tags nested inside of it and place a desired value.  For example you would need to got to the height tag within the tower tag and specify a tower height. This is how almost all of the data for a simulation is specified. For the large majority of use cases all you need to do is go to a specific tag and change the value. You don’t need to worry about anything like range, validset, etc. The process is slightly more complicated for two tags "orbital_properties" and "configuration". "orbital_properties" is the tag concerned with orbit. This tag can have one of two tags nested inside of it, either "fixed_orbit" or "real_orbit". The "child_set" attribute specifies that these are the two tags which are valid tags to nest inside (referred to as children here). You can only pick one of the two. This is because only one kind of orbit makes sense, you can’t have a real and fixed orbit at the same time. In order to specify orbital properties what you will have to do is choose only one of these tags. You can copy and paste one from the validation.xml file and delete the other. 
The only additional complication comes for the “configuration” tag is that it can hold multiple simulation tags. That is you can fill out one simulation tag as described above and place it in the configurations tag. You can then repeat this process for however many simulations you wish. Doing so will cause the program to run multiple simulations each with the specified configuration. 
...
If you have not configured this program before, we highly recommend that you look at the file format outlined in the Appendix. 
 Note that this is a copy of the file validation.xml and you can view the format in that file. This is actually recommended since it will be easier to read if open in a separate file. validation.xml can be copied and used as a template when configuring the simulation. You should only have to perform minimal changes when using this template. The validation.xml file provides comments which specify the parameter and any other applicable information. Essentially all you should have to do is specify values within each of the tags. If this is confusing please refer to the “Overview of XML” secion.

To create a new XML configuration file just make a copy of validation.xml (or copy paste from the Appendix) .
 Enter any desired changes and save the file as follows: <filename>.xml where filename is a filename of your choice that is different from validation.xml.
...
You can place the file anywhere as long as you specify the correct file path relative to Main.py when running the program. To keep things simple it may be a good idea to place your XML file in the same directory as Main.py. This will make it easy to specify the file path later.
...
3) After the prompt appears, type the file path of the xml file you wish to use for configuration. If the file is in the same directory as Main.py you can just type its name. If this is your fist time running the simulation you can try running it with the demo file. To do so simply type “demo.xml” when asked for the file name.
...
You can always refer to validation.xml if you are unsure how something should be formatted. If you still can’t get your simulation running then remake the XML file. Create a copy of validation.xml and enter in your values again.
...
<panel_settings datatype="NA"><!-- Information on the solar panel dimensions used for the simulation -->
   <panel_length datatype="float" range="0.0 - inf"></panel_length>
   <panel_width datatype="float" range="0.0 - inf"></panel_width>
</panel_settings>
